...
async def monthly_matching_job(bot: Bot):
    """Job: Run monthly matching on the 1st of each month."""
    ...
    try:
        ...
        logger.error(f"Monthly matching failed: {e}", exc_info=True)


async def monthly_followup_job(bot: Bot):
    """Job: Send follow-up questions on the 7th of each month (1 week after matching)."""
    ...
    try:
        ...
        logger.error(f"Monthly follow-up job failed: {e}", exc_info=True)


async def inactivity_check_job(bot: Bot):
    """Job: Remove inactive users (3+ consecutive misses)."""
